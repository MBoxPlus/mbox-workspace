...
class StartFromFeatureTests: MBoxUnitTests {
    ...
    func testSimple() {
        ...
        expect(UI.workspace!.status()) == [
            MBWorkspace.Status(name: "mbox-ruby", current: "feature/feature2", target: "master"),
    ...
    func testNoCheckoutFromRemote() {
        let repo = self.currentFeature.findRepo(name: "mbox-ruby")!
        let cmd = MBCMD(workingDirectory: repo.path!)
        cmd.exec("git branch -f master 0e1f8de")

        start(feature: "feature2",
              args: ["--no-checkout-from-remote"],
              status: [MBWorkspace.Status(name: "mbox-ruby", current: "feature/feature2", target: "master")])
