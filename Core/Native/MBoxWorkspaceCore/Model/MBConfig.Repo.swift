...
extension MBConfig {
    ...
    public class Repo: MBCodableObject {
        ...
        open var workspace: MBWorkspace = MBProcess.shared.workspace!
        ...
        public func clone(recurseSubmodules: Bool = false) throws {
            ...
            var opts = GitHelper.CloneOptions.default()
            opts.checkout = false
            opts.recurseSubmodules = recurseSubmodules
            try git.clone(options: opts)
...
extension MBConfig.Repo: MBConfigFeatureExportProtocol {
    dynamic
    public var exportKeys: [String]? {
        guard let ks = self.exportHash?.keys else { return nil }
        var keys = Array(ks)
        keys.removeAll("path")
        keys.removeAll("full_name")
        keys.removeAll("owner")
        keys.removeAll("name")
        return keys
    }
